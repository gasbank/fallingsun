...
import pygame.locals  # @UnusedImport
import os, sys
...
class world(actor):
    ...
    def testForCollision(self, x, y, item, otherItems=[]):
        ...
        if x < 0 or x + item.width > swidth:
            return self.channel
        elif y < 0 or y + item.height > sheight:
            return self.channel
        else:
    ...
    def runFrame(self):
        ...
        while 1:
            ...
            if percentUtilized >= 1:
                if self.updateRate > 1:
                    self.updateRate -= 1
                print "TOO MUCH LOWERING FRAME RATE: " , self.updateRate, " (# of Actors = ", len(self.registeredActors), ")"
            ...
            # print "# of Actors = ", len(self.registeredActors)
...
class spawner(actor):
    ...
    def defaultMessageAction(self, args):    
        ...
        if msg == "WORLD_STATE":
            ...
            elif WorldState.time >= self.time + 1.0:  # every five seconds
                self.time = WorldState.time + 0.2
