...
class SWorld(SActor):
    ...
    def printHavestResult(self):
        ...
        for actor in list(self.registeredActors):
            ...
            actor.send((self.channel, "IDENTIFY_HAVEST_RESULT"))
...
class SWoodcutter(SActor):
    ...
    def havestWood(self, target):
        
        while 1:
            print self.channel, "--HAVEST-->", target, target.balance, target.closed, target.closing
            target.send((self.channel, "HAVEST"))
            
            self.havestingChannel.receive()
            
            havestStart = time.clock()
            havestInterval = 0.1
            while time.clock() < havestStart + havestInterval:
                stackless.schedule()
...
for i in range(30):
    SWoodcutter((random.randrange(50,450),random.randrange(50,450)),
                ...
                velocity=500+(random.random()-0.5)*3,
                instanceName="Woodcutter #%d" % i
                )
